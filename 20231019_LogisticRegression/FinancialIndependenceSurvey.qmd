...
reddit_finance_sub$fin_indy <- as.factor(reddit_finance_sub$fin_indy)
...
reddit_finance_sub <- reddit_finance_sub %>%
  mutate(total_assets = rowSums(select(., 
    c("home_value","brokerage_accts_tax","retirement_accts_tax","cash","invst_accts","spec_crypto","invst_prop_bus_own","other_val","2020_gross_inc","2020_invst_save"))))


#total_debts
reddit_finance_sub <- reddit_finance_sub %>%
  mutate(total_debts = rowSums(select(., 
    c("student_loans","mortgage","auto_loan","credit_personal_loan","medical_debt","invst_prop_bus_own_debt","other_debt","2020_debt_repay"))))


#total_expenses
reddit_finance_sub <- reddit_finance_sub %>%
  mutate(total_expenses = rowSums(select(., 
    c("2020_housing_exp", "2020_utilities_exp", "2020_transp_exp", "2020_necessities_exp", "2020_lux_exp", "2020_child_exp", "2020_charity", "2020_healthcare_exp","2020_taxes", "2020_edu_exp", "2020_other_exp"))))


# We finally removed some previously transformed variables that we will not be using in their original form.
reddit_finance_sub <- reddit_finance_sub[, !(names(reddit_finance_sub) %in% c("political", "race_eth", "gender", "edu", "rel_status", "children","country", "industry","ne_status"))]
```
...
```{r}
ggplot(reddit_finance_sub, aes(x = fin_indy, fill = fin_indy)) +
  geom_bar() +
  labs(x = "Respuesta", y = "Frecuencia", title = "Distribución de Respuestas") +
