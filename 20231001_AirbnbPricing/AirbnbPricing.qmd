...
```{r, echo=FALSE}
#par(mfcol = c(1, 2))
#plot(model1, which = 2, main = "Model 1: using price")
#plot(model2, which = 2, main = "Model 2: using log(price)")
```
...
*ln(price) = 3.743 + 0.129\*(is entire home apt) + 0.082\*(n° bedrooms) + 0.142\*(n° bathrooms) - 0.048\*(dist to downtown) + 0.192\*(has entertainment amenities) + 0.443\*(has climate control) + 0.07\*(n°accommodates)*
...
*ln(price) = 3.743 + 0.129\*(1) + 0.082\*(3) + 0.142\*(1) - 0.048\*(3) + 0.192\*(0) + 0.443\*(1) + 0.07\*(5) = 4.909*
...
-   Room type (which was transformed into the binary variable '**entire_home_apt**' )
...
cat("Mean Squared Error:", mean(model_summary$residuals^2), "\n")
```
